/**
 * Deletes all emojiPluginTest documents created during e2e tests.
 *
...
export default async function globalTeardown() {
  const env = loadEnv(resolve(process.cwd(), 'dev/.env'))
  ...
  // Query for all emojiPluginTest document IDs (published + drafts)
  const queryRes = await fetch(
    `${base}/query/${dataset}?query=${encodeURIComponent('*[_type == "emojiPluginTest"]._id')}`,
